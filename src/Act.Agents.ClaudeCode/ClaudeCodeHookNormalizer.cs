...
public sealed class ClaudeCodeHookNormalizer : IHookNormalizer
{
    ...
    // `Notification` is the only thing that reports a waiting prompt now that ACT does not read
    // the screen, and it is not self-describing: the same event carries permission requests and
    // plain idle nudges. The message is all there is to go on, so ACT reports a permission
    // request when it can tell and an activity observation when it cannot — never a guess that
    // would strand a card in Needs feedback.
    private static AgentEvent Notification(JsonElement payload, string sessionId, DateTimeOffset at)
    {
        ...
        if (!looksLikePermission)
            return new ActivityObserved(sessionId, at);
